...
**1st Step:** 
* Read train, test, features, and activity files and create tables
* Variables: `x_train`, `y_train`, `x_test`, `y_test`, `subject_train`, `subject_test` and `activity_labels` contain the data from the downloaded files.

**2nd Step:**
* Concatenate rows of data to have feature, activity and subject tables
* Variables `subject_table`, `activity_table` and `features_table` created with `rbind` using tables from 1st Step.

**3rd Step:**
* Set the name of the columns of the tables from 2nd Step and `activity_labels` from 1st Step

**4th Step:** 
* Merge all the tables
* Variable: `full_data` created using `cbind` with tables from 2nd Step

**5th Step:**
* Extract only the measurements on the mean and standard deviation for each measurement using `grep` command
* Variables: `subset_data` created subsettig `full_data` from 4th Step with the extracted feature names of `grep`
...
* Variables: `tidy_data` created with `aggregate` command on `subset_data`
...
There are 10,299 observations along 563 features.
