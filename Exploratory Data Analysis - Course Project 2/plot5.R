...
balt_vehicle <- baltimore[which(baltimore$SCC %in% scc_vehicle),]

total_by_year <- group_by(balt_vehicle, year) %>% 
                 summarize(total = sum(Emissions))

# Create PLOT N°5
library(ggplot2)
plot5 <- ggplot(total_by_year, aes(factor(year), total))
         + geom_bar(stat="identity", fill=c(2:5))
         + ggtitle("Total emissions from motor vehicle related sources in Baltimore (1999-2008)")
         + xlab("Year")
